...
UIGen2 is a Next.js 15 App Router app where users chat with Google Gemini AI to generate React components, which are previewed live in the browser without writing files to disk.

### Core Data Flow

1. **Chat** (`/api/chat`) — User sends a message → the server calls Gemini (gemini-2.0-flash via `@ai-sdk/google`) with two tools: `str_replace_editor` and `file_manager`. Gemini uses these tools to create/edit files in the virtual file system.
...
| `src/lib/provider.ts` | `getLanguageModel()` — returns real Gemini or `MockLanguageModel` when no API key |
...
When `GOOGLE_GENERATIVE_AI_API_KEY` is not set, `getLanguageModel()` returns `MockLanguageModel` which streams pre-canned component code (counter, form, or card) to allow local development without an API key.
